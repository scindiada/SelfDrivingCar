...
def generator_images(data, batchSize = 32):
    while True:
        ...
        for i in range(0, len(data), int(batchSize/4)):
            ...
            try:
                for line in details:
                    image = crop_and_resize_change_color_space(cv2.imread('/home/workspace/CarND-Behavioral-Cloning-P3/data/IMG/'+ line[0].split('/')[-1]))
                    ...
                    X_batch.append(crop_and_resize_change_color_space(cv2.imread('/home/workspace/CarND-Behavioral-Cloning-P3/data/IMG/'+ line[1].split('/')[-1])))
                    y_batch.append(steering_angle+0.4)
                    # appending right camera image and steering angle with offset
                    X_batch.append(crop_and_resize_change_color_space(cv2.imread('/home/workspace/CarND-Behavioral-Cloning-P3/data/IMG/'+ line[2].split('/')[-1])))
